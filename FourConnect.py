...
player1 = input("da nume jucator 1 ")
player2 = input("da nume jucator 2 ")
...
def play(x,y):
    ...
    for i in range(len(cols)):
        if abs(x-cols[i]) < 900/14*2/3 and board[ROWS-1][i]==0:
            ...
            if r==0:
                screen.textinput('Nici un loc liber','Remiza')
            elif r==1:
                screen.textinput('Felicitari, a castigat', player1)
            elif r==-1:
                screen.textinput('Felicitari, a castigat', player2)
            if r!=-2: screen.bye()
            turn = -turn
    working = False
